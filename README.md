...
그래서 **어떻게 하면 `event객체`말고 다른 argument(`moleEls[randomNum]`)를 `Event Handler`에 넘길 수 있을까?** 를 고민했습니다.<br/>
<br/>
...
`Event Handler`에서 **`moleEls[randomNum]`이 다루는 태그와 동일한 태그**를 `querySelector`를 통해서 가져온 후 처리하는 방식으로 구현했습니다.
